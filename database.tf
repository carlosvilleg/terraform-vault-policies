...
locals {
  ...
  database_engine_policies = { for a in setproduct(keys(data.local_file.database_engine_policy_files), keys(local.database_engines)) : "${element(a, 1)}-${data.local_file.database_engine_policy_files[element(a, 0)].filename}" => merge({ basename = replace(element(a, 0), ".hcl.tftpl", "") }, local.database_engines[element(a, 1)]) }
  database_role_policies   = { for a in setproduct(keys(data.local_file.database_role_policy_files), keys(local.database_roles)) : "${element(a, 1)}-${data.local_file.database_role_policy_files[element(a, 0)].filename}" => merge({ basename = replace(element(a, 0), ".hcl.tftpl", "") }, local.database_roles[element(a, 1)]) }
}
