...
data "local_file" "engine_policy_files" {
...
data "local_file" "role_policy_files" {
...
locals {
	database_engines = {for a in flatten([for k,v in var.database_roles: [for d,r in v: {engine = k, db=d}]]): "${a.engine}-${a.db}" => a}
	database_roles = {for a in flatten([for k,v in var.database_roles: [for d,r in v: [for l in r: {engine = k, role=l, db=d}]]]): "${a.engine}-${a.db}-${a.role}" => a}
	database_engine_policies = {for a in setproduct(keys(data.local_file.engine_policy_files), keys(local.database_engines)): "${element(a,1)}-${data.local_file.engine_policy_files[element(a,0)].filename}" => merge({basename=replace(element(a,0),".hcl.tftpl", "")},local.database_engines[element(a,1)])}
	database_role_policies = {for a in setproduct(keys(data.local_file.role_policy_files), keys(local.database_roles)): "${element(a,1)}-${data.local_file.role_policy_files[element(a,0)].filename}" => merge({basename=replace(element(a,0),".hcl.tftpl", "")},local.database_roles[element(a,1)])}
}
